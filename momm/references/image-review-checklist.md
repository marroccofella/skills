...
6. Always run the correct-image negative control on the first live run of this gate;
   afterwards, run it if the prompt, adapter, route or model was changed, or if the attachment
   path or container handling changed: those decide which bytes reach the provider, so they can
   substitute one image for another just as a prompt change can. Reproduce and
