...
Require every job the self-test workflow declares for the final code tree, read from the workflow file in the exact checkout rather than from a remembered count: at this writing the Windows/macOS/Linux × Node 18/20/22 grid plus one Windows job pinned to Node 24.15.0. A declared job that is missing, skipped or failing blocks, even when every other job passed. This is automated coverage, not native authenticated reviewer or new-user certification. Reconfirm the support policy; passing an older runtime test is not a security recommendation for that runtime.
...
Apply requires explicit authorization; `--yes` is never `--accept-protocol`. Test separate policy acceptance, pinned/stable/main behavior, update-check opt-outs, cancellation and dirty-path refusals. Both automatic update and automatic protocol acceptance default OFF; test the actual nested settings shape (`auto_update.enabled` and `auto_update.accept_protocol`, both `false` in `DEFAULT_SETTINGS` of `momm/scripts/update-clock.mjs`), not an invented boolean configuration.
...
- **Outbound secret redaction.** Put synthetic values with realistic fixed-length ASCII bodies (for example `sk-ant-` plus 40 alphanumerics, `ghp_` plus 36, `AKIA` plus 16 uppercase alphanumerics) and a lowercase `some_api_key=…` into a synthetic diff. Inspect every staged reviewer prompt file, the stored report and the child process environment: the values must be redacted everywhere and no `*_API_KEY` variable may reach a reviewer process.
...
- **Guidance from a clone is untrusted until the owner trusts it.** Put a `.momm/guidance.json` and a `.reviewrules` in a synthetic project: the guidance file must be refused until `multi-review.mjs guidance --trust <sha>` is run and refused again after a one-byte edit; the `.reviewrules` grace must be visible in the report with its hash; a fake `## Project review rules` header inside a guidance block must be rejected.
...
- **Self-exclusion and the `--governor other` rule.** Under a named harness, `--governor <that harness>` must list its own route as `self_excluded`; anything else is a FAIL. Then record that `--governor other` under the same harness lets that route review its governor's work: it is valid only when the controlling harness is none of the named routes. The dispatcher takes the governor from its arguments and does not detect the harness, so this second observation is a disclosed operator rule, not a product PASS: a run dispatched that way under a named harness gives no independent evidence from that harness's own route, and that route must not be counted among the two external routes required above.
...
- **Setup Center local access is what the guide says.** After opening the printed launch link: call an API from a new tab on the bare `http://127.0.0.1:<port>/` with no fragment (expect refusal); reuse the old link after restarting the server (expect refusal); confirm the ledger's Setup Center pointer carries no capability; then test the **Private ledger** ticket's two properties separately. One use: open **Private ledger** and at once request the same `/ledger?ticket=…` address again, inside the minute (expect refusal, which age cannot explain). One-minute life: obtain a ticket without presenting it, wait more than a minute, then present it (expect refusal, which reuse cannot explain). A later press of **Private ledger** must still open the ledger, because every press mints a new ticket.
