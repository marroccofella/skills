...
Cheapest possible Kimi route: **Copilot CLI 1.0.83 lists `kimi-k3` and `kimi-k2.7-code`** (also `grok-4.5`, `gemini-3.8-flash`, `mai-code-1.1-flash`) under the existing GitHub login — no new CLI, no new auth. On 2026-09-13 `--model kimi-k3` returned "not available" on this plan while the quota was exhausted, so it is unproven; re-probe when the quota resets (see [copilot.md](copilot.md)). If it works it shares Copilot's quota, persona and containment, so it is a model swap, not an independent route.
