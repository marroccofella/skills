...
Captured 2026-09-12 on Windows 11; help text re-captured 2026-09-13 after every CLI except Antigravity was upgraded (versions below are the current ones). Re-capture with the commands at the end whenever a CLI is updated; a flag that disappears from `--help` is a fail-closed change for its adapter.

| Route | Binary | Installed version | Page |
| --- | --- | --- | --- |
| codex | `codex` (npm `@openai/codex`) | codex-cli 0.154.0 | [codex.md](codex.md) |
| claude | `claude` (npm `@anthropic-ai/claude-code`) | 2.1.270 | [claude-code.md](claude-code.md) |
| antigravity | `agy` (native installer) | 1.2.2 | [antigravity.md](antigravity.md) |
| copilot | `copilot` (npm `@github/copilot`) | 1.0.83 | [copilot.md](copilot.md) |
| grok | `grok` (`~/.grok/bin/grok`, x.ai installer) | 1.0.30 stable | [grok.md](grok.md) |
| gemini | `gemini` (npm `@google/gemini-cli`) | 0.59.0 | [gemini.md](gemini.md) |
...
| Disable tools entirely | n/a (sandbox governs) | `--tools ""` (verified: tool call returned as text, no read); 2.1.270 adds `--restricted` (verified: `NO-TOOLS`, one turn) | not documented | `--available-tools` filter | **`--tools ""` and `--tools none` do NOT disable** (verified on 1.0.5 and 1.0.30: read a canary file). `--disallowed-tools read_file,search_replace,…` does stop the read (verified 1.0.30; run ends `cancelled`); the 1.15.0 vector `--deny Read --deny Grep --deny Bash … --max-turns 4` also stops it and ends `end_turn` in 3 turns (verified 1.0.30) | policy engine only |
...
## Bounded Windows shutdown mitigation

On 2026-09-16 an independent reviewer reproduced `UV_HANDLE_CLOSING` on
Windows / Node24.15.0 after a completed public-manifest fetch followed by an
immediate forced exit, without importing MOMM. Natural exit succeeded; four
...
survives stalled or synchronously broken output pipes. POSIX retains immediate
post-flush exit. Owned-child tree cleanup, direct-kill backstops and signal
handling are unchanged. A forced fallback may still encounter the runtime bug;
bounded termination is retained rather than silently waiting forever.

`shutdown.test.mjs` tests the exact production finalizer with synthetic clocks
and real piped loopback fetches. These are not external-provider or cross-account
tests. A passing older runtime does not establish Node24 behavior; retest the
actual metadata wrapper and live dispatcher on the affected runtime too.
