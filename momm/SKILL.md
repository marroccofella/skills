...
Quoted scope treats line endings, typographic look-alikes (curly and straight quotes, dash variants, the ellipsis, non-breaking spaces) and runs of whitespace as equal; every other character must match literally, and a `+` diff marker is always kept (a context line's leading space counts as whitespace, and a dash look-alike as `-`). Report/input hashes still bind the original sanitized bytes. This is a narrow, listed equivalence, not fuzzy quote matching or proof of the reviewer's reasoning.
