...
for (const alias of ['C:\\junction', 'C:\\symlink', 'C:\\PROJEC~1']) {
  ...
  assert.equal(windowsChildEnv(env,{cwd:root,fs:files,platform:'win32'}).PATH,trusted);
  assert.equal(windowsTool('taskkill',{cwd:root,env,platform:'win32',fs:files}),'C:\\Windows\\System32\\taskkill.exe');
}
const files = { statSync: () => ({isFile:()=>true}), realpathSync: {native:p=> {if(p.startsWith('C:\\unresolved')) throw Object.assign(new Error('EACCES: permission denied'),{code:'EACCES'}); return p;}}};
...
assert.equal(windowsChildEnv(env,{cwd:root,fs:files,platform:'win32'}).PATH,trusted,'unresolvable PATH entry must be removed');
