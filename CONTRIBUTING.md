...
4. **Deterministic scripts, zero dependencies.** Node 18+ standard library only. Every skill that spawns subprocesses must route them through timeout + process-tree-kill + hard-deadline containment (see `momm/scripts/multi-review.mjs` `runProcess` for the reference implementation).
