...
export const answers = [
  ...
  ['install-or-upgrade', 'How do I install or upgrade MOMM?', 'Use the install or upgrade prompt to identify your harness and any existing installation, verify a published stable release, preview the changes and ask before applying them. Keep the clone in a permanent folder. Protocol changes require separate acceptance; automatic updates are off by default and require owner opt-in.', 'releases/upgrade.html', 'Copy the install or upgrade prompt'],
...
export function pageMetadata(file, catalogue) {
  const fixed = {
    'docs/momm/index.html': ['MOMM: A Review Team for Your AI Agent', 'MOMM brings peer review to your coding agent through existing CLI logins. One writer, verified findings, explicit decisions and a private local ledger.'],
    ...
    'docs/momm/media.html': ['MOMM Media Gallery: Introductions and Setup Guides', 'Watch approved MOMM introductions and practical setup guides. Each film identifies its recording version, runtime, captions and transcript.'],
    'docs/momm/improvement.html': ['How MOMM Improves: Evidence, Suggestions and Human Decisions', 'Follow public release observations, suggest a reproducible improvement and see how MOMM keeps review, implementation and release approval separate.'],
    ...
    'docs/momm/updates.html': ['Update MOMM Safely: Signed Releases and Rollback', 'Preview a signed MOMM update, inspect protocol changes, choose stable or pinned versions and recover a retained installation. Automatic updates are off by default.'],
