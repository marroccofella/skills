...
// The install front door: one short address a person can read aloud, and one line to paste.
// The page carries the complete verified procedure, so the pasted line never needs a repository path.
export const INSTALL_ONE_LINE = "Install MOMM for me by following https://marroccofella.github.io/skills/momm/install";
function installBody(root) {
...
<p><strong>Two things agents ask.</strong> <em>Which harness?</em> The one you are running in now; if you cannot tell, ask the person. <em>Is the verifier present?</em> Run <code>gitsign --version</code>. If it is missing, that is normal on a first install: say in one or two plain sentences what gitsign is and that it is a one-time step, ask once, then install it exactly as <a href="releases/bootstrap.html#getting-the-verifier-gitsign-once">Getting the verifier</a> says for this system, Windows included. Never continue without it and never fetch it from anywhere but the official <code>sigstore/gitsign</code> release.</p>
...
export function pages(data, s, version) {
  ...
  const start = `${hero("GET STARTED", "A first review,<br><span>without the guesswork.</span>", "Install only what you need. Keep the project you want reviewed as the working directory. Let your agent close the review loop.")}
    ...
    <section id="install"><h2><span class="step">01</span> Verify, then install only MOMM</h2><p>Use the <a href="releases/bootstrap.html">new-install and legacy bootstrap guide</a> to prepare an exact signed release in a permanent directory before executing its code. The separately trusted bootstrap helper checks tools first and never installs or replaces an existing link. Keep the verified clone: your harness will link to it.</p>
    ${note("Missing verifier is not a bad signature", "Ask before installing gitsign; on macOS with Homebrew, its official command is <code>brew install gitsign</code>. GitHub’s <code>bad_cert</code> / Unverified badge is not a gitsign result. Verify the expected signer, transparency record and package hash; stop on any actual failure.")}
  ...
  const updates = `${hero("UPDATE SAFELY", "A newer release is news.<br><span>Not permission.</span>", "Every code change is initiated in your terminal. Review the plan, accept the policy change if there is one, and explicitly choose to install.")}
    ...
    ${note("Choose the right upgrade path", "New installation, missing gitsign or an old release without <code>momm.lock</code>? Start with the <a href='releases/bootstrap.html'>bootstrap guide</a>. It separates missing tools from failed signatures and preserves the existing clone. GitHub’s <code>bad_cert</code> badge is not a gitsign verdict. The 1.15.0 staging defect requires a separately verified newer updater. Older unsigned releases are never trusted retroactively.")}
...
export function renderPublic({ root = ROOT, check = false, sourceData } = {}) {
  ...
  const banner = prefix => `<aside class="stable-banner" aria-label="${published?'Current stable release':'Checkout version; publication not recorded'}"><a href="${prefix}releases/${esc(version)}.html"><span class="dot"></span> ${published?'CURRENT STABLE':'CHECKOUT VERSION · CHECK PUBLICATION'} <strong>MOMM ${esc(version)}</strong></a><a href="${prefix}install.html">Install in one line →</a></aside>`;
