...
function fixture(name){const dir=path.join(temp,name);fs.mkdirSync(path.join(dir,'docs/evidence'),{recursive:true});fs.cpSync(path.join(root,'momm/references'),path.join(dir,'momm/references'),{recursive:true});fs.copyFileSync(path.join(root,'versions.json'),path.join(dir,'versions.json'));
 fs.mkdirSync(path.join(dir,'docs/momm'),{recursive:true});const fixtureTour=JSON.parse(fs.readFileSync(path.join(root,'docs/momm/tour.json')));fixtureTour.status='pending_voice_acceptance';fs.writeFileSync(path.join(dir,'docs/momm/tour.json'),JSON.stringify(fixtureTour));
 fs.writeFileSync(path.join(dir,'docs/momm/films.json'),'[]');
 fs.writeFileSync(path.join(dir,'docs/index.html'),'<html><span data-momm-version>fixture</span></html>');
 fs.writeFileSync(path.join(dir,'docs/evidence/index.html'),'<!doctype html><html><head><title>Public fixture ledger</title></head><body><script id="data" type="application/json">{}</script></body></html>');fs.writeFileSync(path.join(dir,'docs/evidence/momm-evidence.json'),'original public bytes');return dir;}
