...
try{
 ...
 test('legacy sparse report renders without reviewers',()=>{const dir=fixture('sparse'),er=input(dir,'er',[{run_id:'rev_fixture',reviewer_status:{}}],{input_bytes:10});const r=run(dir,[er]);assert.equal(r.status,0,r.stderr);assert.match(fs.readFileSync(path.join(dir,'docs/momm/data/input-size-vs-time.csv'),'utf8'),/0,0,0,0/);});
